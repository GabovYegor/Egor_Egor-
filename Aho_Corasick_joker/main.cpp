...
int main(){
    ...
    char separate_sim = '\0';
    std::cin >> main_str >> pattern_str >> separate_sim;
    std::string temp_str;
    for(size_t i = 0, separate_index = 0, pattern_num = 0; i < pattern_str.size(); ++i){
        if(pattern_str[i] == separate_sim){
            temp_str = pattern_str.substr(separate_index, i - separate_index);
            separate_index = i + 1;
            if(temp_str.size())
                //std::cout << temp_str << std::endl;
                add_str_to_bor(bohr, temp_str, ++pattern_num);
        }
        if(i == pattern_str.size() - 1){
            temp_str = pattern_str.substr(separate_index, i - separate_index + 1);
            if(temp_str.size())
                //std::cout << temp_str << std::endl;
                add_str_to_bor(bohr, temp_str, ++pattern_num);
        }
